...
replace_unlikely_values <- function(measurements, min_value, max_value) {
  # Use case_when to replace values outside bounds with NA
  measurements <- case_when(
    measurements < min_value | measurements > max_value ~ NA_real_,
    TRUE ~ measurements
  )
  return(measurements)
}
...
data <- data |>
  mutate(
    length = replace_unlikely_values(length, 5, 80),
    weight = replace_unlikely_values(weight, 10, 4000)
  )
```
...
calculate_condition_index <- function(weight, length) {
  # Calculate condition index using the given formula
