...
# Read the CSV file
data <- read.csv(here("data", "BlackfootFish.csv"))
```
...
The portion of the code that calculates the normalized values of length and weight are exactly the same and thus repeated. This makes errors hard to spot and is efficient code. A function should be written to perform this task.
...
rescale_column(data, c("length", "weight"))
```
